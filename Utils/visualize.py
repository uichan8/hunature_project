...
def plot_tracking(im, tlwhs, obj_ids, centroids, count, fps, ids2=None):
    ...
    for i, tlwh in enumerate(tlwhs):
        ...
        cv2.putText(im, id_text, (intbox[0], intbox[1]), cv2.FONT_HERSHEY_PLAIN, 3, (0, 0, 255),
                    thickness=3)
    
    #화면 맨 위에 뜨는 정보 텍스트
    cv2.putText(im, 'fps: %.2f count: %d' % (fps, count),(0, 30), cv2.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), thickness=2)
    return im
